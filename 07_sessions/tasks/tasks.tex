...
\section{session\_regenerate\_id()}

Ergänzen Sie den Login aus Aufgabe 6 um einen Aufruf von \texttt{session\_regenerate\_id(true)}, bevor Sie \texttt{\$\_SESSION["{}username"{}]} setzen. Geben Sie vor und nach dem Login jeweils \texttt{session\_id()} aus und vergleichen Sie die beiden IDs im Browser.

\section{Session-Timeout}

Ergänzen Sie direkt nach \texttt{session\_start()} folgende Prüfung: Ist \texttt{\$\_SESSION["{}lastActivity"{}]} gesetzt und liegt der letzte Zugriff (\texttt{time() - \$\_SESSION["{}lastActivity"{}]}) mehr als \texttt{15} Sekunden zurück, rufen Sie \texttt{session\_destroy()} und danach erneut \texttt{session\_start()} auf, um mit einer leeren Session weiterzufahren. Setzen Sie am Ende dieses Blocks \texttt{\$\_SESSION["{}lastActivity"{}] = time()}. Testen Sie das Verhalten, indem Sie die Seite einmal sofort und einmal nach mehr als 15 Sekunden Wartezeit neu laden.

\section{Flash-Message}

Setzen Sie beim erfolgreichen Login aus Aufgabe 6/10 zusätzlich \texttt{\$\_SESSION["{}flash"{}] = "{}Login erfolgreich!"{}}. Geben Sie diese Nachricht - falls vorhanden - ganz am Anfang Ihrer Ausgabe aus und entfernen Sie sie danach sofort mit \texttt{unset()}, damit sie beim nächsten Neuladen nicht mehr erscheint.
